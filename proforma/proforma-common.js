...
const proformaId = params.get("id");
let lines = [];
let account = null;
...
async function loadProforma() {
  ...
  const pf = await fetch(`/api/proformas/${proformaId}`).then(r => r.json());
  document.getElementById("cliente_nombre").value = pf.cliente_nombre || "";
  document.getElementById("cliente_rtn").value = pf.cliente_rtn || "";
  selectedClienteId = pf.cliente_id || null;
  document.getElementById("fecha").value = pf.fecha || "";
  document.getElementById(pf.termino_pago === "credito" ? "term-credito" : "term-contado").checked = true;
  document.getElementById("descuentos").value = pf.descuentos || 0;
  document.getElementById("importe_exonerado").value = pf.importe_exonerado || 0;
  document.getElementById("importe_exento").value = pf.importe_exento || 0;
  document.getElementById("gravado_18_pct").checked = (pf.importe_gravado_18 || 0) > 0;
  document.getElementById("orden_compra_exenta").value = pf.orden_compra_exenta || "";
  document.getElementById("constancia_registro_exonerado").value = pf.constancia_registro_exonerado || "";
  document.getElementById("registro_sag").value = pf.registro_sag || "";
  lines = pf.lines.map(l => ({ _key: uid(), cantidad: l.cantidad, descripcion: l.descripcion, precio_unitario: l.precio_unitario }));
...
document.getElementById("btnGuardar").addEventListener("click", async () => {
  ...
  try {
    ...
    if (!proformaId) {
      window.location.href = `/proforma/ver/?id=${data.id}`;
    } else {
...
document.getElementById("btnImprimir").addEventListener("click", () => {
  if (proformaId) {
    window.open(`/api/proformas/${proformaId}/pdf`, "_blank");
  } else {
    alert("Guarda la proforma primero para generar el PDF.");
  }
});
